const CRITICAL_DISCARD_FIELDS = new Set(['age', 'documentNumber', 'gender']);
const GREETING_REGEX = /^(hola|buenas|buenos\s+dias|buenas\s+tardes|buenas\s+noches|mucho\s+gusto)/i;
const ADDRESS_AGE_REGEX = /\b(calle|cra|carrera|avenida|av\.?|mz|manzana|torre|apto)\s*\d+/i;
...
export function applyFieldPolicy(extraction = {}, currentCandidate = {}) {
  ...
  for (const [field, value] of Object.entries(fields)) {
    ...
    if (field === 'fullName' && GREETING_REGEX.test(String(value))) {
      blocked.push({ field, reason: 'greeting_as_name' });
      continue;
    }

    if (field === 'age' && ADDRESS_AGE_REGEX.test(String(evidence.snippet || ''))) {
      blocked.push({ field, reason: 'address_as_age' });
      continue;
    }

    if (!hasStrongEvidence(evidence)) {
      reviewQueue.push({ field, value, evidence });
